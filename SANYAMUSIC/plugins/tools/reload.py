...
from SANYAMUSIC.utils.database import get_assistant, get_authuser_names, get_cmode
from SANYAMUSIC.utils.decorators import ActualAdminCB, AdminActual, language
from SANYAMUSIC.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await SANYA.stop_stream_force(chat_id)
        except:
            pass
             ...
             [
                 [
                      InlineKeyboardButton(
                         "• нαϲкє𝚍 ву  •", url=f"https://t.me/urstarkz")
                 ]
            ]
         ),
     )
